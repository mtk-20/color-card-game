...
public class GameServiceImpl implements GameService {
    ...
    @Override
    @Transactional
    public GameResponse vipMode(GameRequest request) {
        ...
        List<VipDailyPrize> prizes = vipDailyPrizeRepo.findByDay(dayNo);
        List<VipDailyPrize> availablePrizes = prizes.stream().filter(a -> a.getAvailableQuantity() > 0).toList();
        ...
        VipDailyPrize chosenPrize = pickRandomPrizeVip(availablePrizes, prizeRank);
        ...
        vipDailyPrizeRepo.save(chosenPrize);
    ...
    @Override
    @Transactional
    public GameResponse normalMode(GameRequest request) {
        ...
        List<NormalDailyPrize> prizes = normalDailyPrizeRepo.findByDay(dayNo);
        List<NormalDailyPrize> availablePrizes = prizes.stream().filter(a -> a.getAvailableQuantity() > 0).toList();
        ...
        NormalDailyPrize chosenPrize = pickRandomPrizeNormal(availablePrizes, prizeRank);
        chosenPrize.setAvailableQuantity(chosenPrize.getAvailableQuantity() - 1);
        ...
        prizeRepo.save(prize);
        normalDailyPrizeRepo.save(chosenPrize);
    ...
    private NormalDailyPrize pickRandomPrizeNormal(List<NormalDailyPrize> prizes, int rank) {

        List<NormalDailyPrize> filtered = prizes.stream()
                .filter(p -> p.getPrize().getPrizeRank() == rank)
                .filter(p -> p.getAvailableQuantity() > 0).toList();

        if (filtered.isEmpty()) {
            throw new CommonException("ERR_500", "No Normal Prize Left For Rank " + rank);
        }

        int dropRate = filtered.stream().mapToInt(NormalDailyPrize::getAvailableQuantity).sum();
        int randomNumber = random.nextInt(dropRate) + 1;
        int cumulative = 0;

        for (NormalDailyPrize prize : filtered) {
    ...
    private VipDailyPrize pickRandomPrizeVip(List<VipDailyPrize> prizes, int rank) {

        List<VipDailyPrize> filtered = prizes.stream()
                .filter(p -> p.getPrize().getPrizeRank() == rank)
                .filter(p -> p.getAvailableQuantity() > 0).toList();

        if (filtered.isEmpty()) {
            throw new CommonException("ERR_500", "No Vip Prize Left For Rank " + rank);
        }

        int dropRate = filtered.stream().mapToInt(VipDailyPrize::getAvailableQuantity).sum();
        int randomNumber = random.nextInt(dropRate) + 1;
        int cumulative = 0;

        for (VipDailyPrize prize : filtered) {
            cumulative += prize.getAvailableQuantity();
            if (randomNumber <= cumulative) {
                return prize;
            }
